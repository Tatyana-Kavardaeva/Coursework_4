...
class Vacancy:
    ...
    def __init__(self, name='', salary=None, area=None, url=None):
        self.name = name
        ...
        if salary is None or salary == 'Зарплата не указана':
            ...
            range_salary = re.split(r'\s|-', salary)
            self.salary_min = int(range_salary[0])
            self.salary_max = int(range_salary[1])
            self.currency = range_salary[2]
            if isinstance(self.salary_min, int):
                self.salary_min = self.salary_min
            else:
                self.salary_min = 0
    ...
    def __repr__(self):
        return f"Vacancy(name=f'{self.name}', salary_min={self.salary_min}, salary_max={self.salary_max}, " \
               f"currency=f'{self.currency}', area=f'{self.area}', url=f'{self.url}')"
